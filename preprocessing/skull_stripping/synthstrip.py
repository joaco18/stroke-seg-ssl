...
def predict(
    ...
    if (not out_path) and (not mask_path):
        raise Exception(
            'Must provide at least --out or --mask output flags.')
    ...
    # configure model
    logging.debug(f'Configuring model on the {device_name}')
    with torch.no_grad():
        model = StripModel()
        model.to(device)
        model.eval()
    ...
    if model_path is not None:
        ...
        logging.debug('Using custom model weights')
    else:
        version = '1'
        logging.debug(f'Running SynthStrip model version {version}')
        if no_csf:
            logging.debug('Excluding CSF from brain boundary')
            modelfile = this_file_path / f'models/synthstrip.nocsf.{version}.pt'
            modelfile = str(modelfile)
        else:
            modelfile = str(this_file_path / f'models/synthstrip.{version}.pt')
    ...
    logging.debug(f'Input image read from: {str(image_path)}')

    # frame check
    if image.nframes > 1:
        raise Exception('Input image cannot have more than 1 frame')

    # conform image and fit to shape with factors of 64
    conformed = image.conform(
        voxsize=1.0, dtype='float32', method='nearest', orientation='LIA').crop_to_bbox()
    ...
    components = scipy.ndimage.label(sdt.data < border)[0]
    ...
    if out_path:
        ...
        logging.debug(f'Masked image saved to: {out_path}')

    # write the brain mask
    if mask_path:
        image.new(mask).save(mask_path)
        logging.debug(f'Binary brain mask saved to: {mask_path}')

    logging.debug('If you use SynthStrip in your analysis, please cite:')
    logging.debug('----------------------------------------------------')
    logging.debug('SynthStrip: Skull-Stripping for Any Brain Image.')
    logging.debug('A Hoopes, JS Mora, AV Dalca, B Fischl, M Hoffmann.')
    logging.debug('NeuroImage 206 (2022), 119474.')
