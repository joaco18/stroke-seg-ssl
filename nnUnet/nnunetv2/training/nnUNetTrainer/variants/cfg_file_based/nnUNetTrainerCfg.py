...
class nnUNetTrainerCfg(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        # Read the configuration file from nnUNet preprocessed dataset folder
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # Decide what to do with the weights of the encoder if it is pretrained
            ...
            # Check if the model parameters are freezed
    ...
    def on_train_epoch_start(self):
        ...
        if self.unfreeze_lr is not None:
            if lr <= self.unfreeze_lr:
                ...
                # Check that the model parameters were unfreezed
                model_parameters = filter(lambda p: p.requires_grad, self.network.parameters())
                params = sum([np.prod(p.size()) for p in model_parameters])
                total_params = sum([np.prod(p.size()) for p in self.network.parameters()])
    ...
    def on_epoch_end(self):
        ...
        # Use config to save more than two checkpoints
