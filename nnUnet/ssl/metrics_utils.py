...
def log_scatterplot_figure(proj_df: pd.DataFrame, writer: SummaryWriter,
    ...
    palette = {'AISD': 'C1', 'APIS': 'C2', 'TBI': 'C3'}
...
def log_example_images(images: List[np.ndarray], writer: SummaryWriter,
    ...
    plt.tight_layout()
    plt.axis('off')
    writer.add_figure('exp_imgs', fig, global_step=epoch)
    if n_channels == 2:
        fig, ax = plt.subplots(1, 1, figsize=(15, 6))
        ...
        plt.tight_layout()
        plt.axis('off')
        writer.add_figure('exp_imgs_ch2', fig, global_step=epoch)
        ...
        for df, name in zip([embed_df, lat_rep_df], ['bottleneck', 'head']):
            ...
            if log_rankme or log_projections:
                ...
                if log_rankme:
                    ...
                    condition = (name == 'head') and (partition != 'train')
                    if condition and (pca_rank_me > best_rankme) and (epoch > 5):
                    ...
                    log_dict.update({f'{name}_pca_rank_me': pca_rank_me,
                                     f'{name}_np_rank': np_rank})
