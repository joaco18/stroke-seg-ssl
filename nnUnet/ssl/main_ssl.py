'''
Code based on https://github.com/yeerwen/DeSD
@Time    :   2023/06/05 15:18:57
@Author  :   Joaquin Seia
...
import nnUNet.ssl.ssl_utils as utils

from pathlib import Path
from nnUNet.ssl.desd_loss import DeSDLoss
from nnUNet.ssl.models import DINOHead, nnunet_encoder, DynamicMultiCropWrapper
from nnUNet.ssl.data_loader_ssl import Dataset3D
from nnUNet.ssl.metrics_utils import at_epochs_end, log_example_images
...
def train_ssl(cfg: dict):
    ...
    if 'encoder_lr' in cfg.keys():
        ...
        print(f'After freezing - The student has {(params/total_params)*100}%'
    ...
    if 'encoder_lr' in cfg.keys():
        lr_schedule_enconder = utils.cosine_scheduler(
            cfg['encoder_lr'] * cfg['batch_size'] / cfg['lr_sch_den'],  # linear scaling rule
            cfg['min_encoder_lr'],
            cfg['epochs'],
            len(train_data_loader),
            warmup_epochs=cfg['warmup_epochs'])
    ...
    for epoch in range(start_epoch, cfg['epochs']):
        ...
        train_stats = train_one_epoch(student, teacher, desd_loss, train_data_loader,
                                      optimizer, lr_schedule, lr_schedule_enconder,
