...
PALETTE_DS = {"apis": "C1", "aisd": "C2", "tbi": "C3", 'I-AIS': "C0", 'C-TBI': "C3"}
...
def visualize_neighbours(base_exp_path: Path, base_cfg_path: Path, exp_cfg: Dict,
    ...
    Args:
        ...
        img_types (List, optional): _description_. Defaults to
            ['ncct', 'diff', 'attr', 'attr-diff', 'gt'].
    ...
    raw_path = Path(nnUNet_raw) / cfg['dataset'] / 'imagesTr'
    ...
    for pts_idxs in indexes:
        ...
        for pt_idx in pts_idxs:
            ...
            if 'attr' in img_types:
                ...
                if 'diff' in img_types:
                    attr_diff = adapt_size(attr_diff[slice_idx, :, :], destiny_shape[1:],
                                           padd_val=0)
    ...
    for exp_cfg in exps_cfgs:
        ...
        if plot_images:
            ...
            if plot_type == 0:
                ...
                set_axis(ax[1, 2])
        fg.suptitle('Enconder embedings for experiment: '
...
def scatter_examples_method_sr(base_cfg_path, base_experiments_path, exps_cfgs, gt_path: Path,
    ...
    for exp_cfg in exps_cfgs:
        ...
        ax[0].set(xlim=[-25, 42], ylim=[-12, 12], aspect=1.5)
        ax[0].set_ylim([-12, 12])
        ...
        if plot_images:
            ...
            overlay = np.repeat(np.expand_dims(overlay, 0), axis=0,
                                repeats=len(ncct_gt[gt == 255]))
...
def scatter_examples_method_diff(base_cfg_path, base_experiments_path, exps_cfgs, gt_path: Path,
    ...
    for exp_cfg in exps_cfgs:
        ...
        if plot_images:
            ...
            set_axis(ax[1, 2])
        fg.suptitle(f'Low dimensionality projections and attibution maps for {encoder_name}',
                    fontsize=15)
        plt.tight_layout()
