...
from nnUnet.ssl.models import nnunet_encoder, DINOHead, DynamicMultiCropWrapper
from nnUnet.ssl.ssl_utils import save_img_from_array_using_referece
...
class DimReductor():
    def __init__(self, method: str = 'pca', seed: int = 420,
        ...
        if self.method == 'pca':
            fields = ['pca', 'incrementalpca']
            if (self.method in fields) and ('whiten' not in method_kwargs.keys()):
    ...
    def __init__(self, cfg_file_path: Path, chckpt_file_path: Path,
        ...
        if npy_path is None:
            ...
            self.preprocessed_dataset_path = \
                Path(nnUNet_preprocessed) / nnu_dataset_name / f'{nnu_planner}_{nnu_configuration}'
            self.raw_path = Path(nnUNet_raw) / nnu_dataset_name / 'imagesTr'
        else:
